...
<div class="date-time-picker w-full {styling}" on:focusout tabindex="0" on:keydown={keydown}>
  <div class="tab-container" tabindex="-1">
    <div class="top flex">
      <div class="flex px-2 w-full">
        <b class="mr-1 font-semibold">{iLocale.months[browseMonth]}</b>
        {browseYear}
      </div>
      <div class="flex space-x-2 mr-2">
        ...
        >
          <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24"
            ><path fill="gray" d="M5 3l3.057-3 11.943 12-11.943 12-3.057-3 9-9z" /></svg
          >
        </button>
      </div>
    ...
    {#each Array(6) as _, weekIndex}
      <div class="week">
        {#each calendarDays.slice(weekIndex * 7, weekIndex * 7 + 7) as calendarDay}
          ...
          >
            <span class="font-light">{calendarDay.number}</span>
          </div>
        {/each}
      </div>
    {/each}
