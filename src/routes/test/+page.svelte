  ...
  <div class="flex flex-col justify-center max-w-12xl mx-auto text-[#16182F]">
    ...
    <div class="flex pt-4 space-x-2 justify-center">
      <button
        class="{showCalendar
          ...
          : 'text-gray-800 border-slate-800'} border px-3 py-1 rounded"
    ...
    {#if !showCalendar}
      ...
      >
        <div class="lg:flex space-x-4 hidden">
          <div
            class="w-1/2 border shadow pt-6 pb-4 px-4 rounded-xl border-slate-700 bg-slate-900"
          >
            <DatePicker bind:value={fromDate} styling={"text-gray-100"} />
          </div>
          <div
            class="w-1/2 border shadow pt-6 pb-4 px-4 rounded-xl border-slate-700 bg-slate-900"
          >
            <DatePicker bind:value={toDate} styling={"text-gray-100"} />
          </div>
        </div>
          ...
          <div class="flex flex-col space-y-4 lg:w-1/2 mx-auto">
            <div class="flex space-x-2 text-gray-100">
              <div class="flex flex-col w-full space-y-2">
                ...
                <input
                  type="time"
                  class="px-4 pt-2 pb-1.5 w-full rounded-lg border border-slate-700 bg-slate-900"
                  id="start"
                  name="start"
                  placeholder="00:00"
                  step="3600"
            ...
            <div class="flex flex-col space-y-2">
              ...
              <input
                class=" px-4 py-2 text-gray-100 rounded-lg border border-slate-700 bg-slate-900"
                type="text"
                id="title"
                name="title"
                bind:value={title}
        ...
        {:else}
          <div class="text-center">
            <p
              class="bg-[#2d2e31] mt-6 text-gray-500 py-2 px-2 lg:w-1/3 w-full rounded-lg mx-auto shadow"
            >
              Lav reservation
            </p>
          </div>
    ...
    {#if showCalendar}
      ...
      >
        ...
        >
          <h3 class="text-lg md:text-xl text-white">
            <span class="text-white">Næste reservering er foretaget af </span>

            {nearestEvent?.title}
            <span class="text-[16px] text-white flex">{timeUntilNextEvent}</span
            >
          </h3>
          {#if nearestEvent?.start !== "1970-01-01T00:00:00.000Z" && nearestEvent?.end !== "1970-01-01T00:00:00.000Z"}
            <div class="flex text-white space-x-2 text-sm">
              <div class="space-y-0 w-1/2">
                <p>Fra</p>
                <p
                  class="bg-slate-800 text-white px-4 py-2 rounded-lg border border-slate-900"
                >
              ...
              <div class="space-y-0 w-1/2">
                <p>Til</p>
                <p
                  class="bg-slate-800 px-4 text-white py-2 rounded-lg border border-slate-900"
                >
        ...
        {#if chosenEvent && chosenEvent.id}
          <div
            class="mt-4 bg-slate-900 opacity-80 shadow mx-auto text-white px-4 pb-4 py-3 rounded-lg border border-slate-700"
          >
