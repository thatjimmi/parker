...
export async function GET({
    ...
    const pages = ['mailbox', 'blog']
    const posts = []
    for(var propt in blogPosts){
      posts.push({slug: propt, updatedAt: new Date(blogPosts[propt].date).toISOString().split('T')[0]})
    }
